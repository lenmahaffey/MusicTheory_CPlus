	...
	Music::Scale(note, pattern, false)
{
	...
	Music::HarmonicMinorScale::resolveScale();
}
...
Music::HarmonicMinorScale::HarmonicMinorScale(std::string note) :
	Music::Scale(note, pattern, false)
{
	std::copy(std::begin(pattern), std::end(pattern), std::begin(Music::Scale::pattern));
	Music::HarmonicMinorScale::setScale(Music::Position::GetPositionFromChromaticScale(note));
...
void Music::HarmonicMinorScale::resolveScale()
{
	for (int i = 0; i < Music::Scale::scalePatternLength; i++)
	{
		...
		currentNote = Music::HarmonicMinorScale::unresolvedScale[i];
